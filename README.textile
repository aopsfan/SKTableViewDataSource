...
h2. Compared to NSFetchedResultsController
...
h2. Getting SKTableViewDataSource in your project
...
Fire up both your project's @.xcodeproj@ and @SKTableViewDataSource.xcodeproj@. Drag the group titled "SKTableViewDataSource" over to your project (remember that this is a group, and the folder you will see in the Finder doesn't include all of the necessary tools). Uncheck the box that wants to copy files over; if copies are created, they will not be linked to this repo.

In Xcode 4, click on the big cell above all your files in the file viewer. Click on your application's _target_, not _project_. Then select "Build Phases" from the tab bar at the top of the view. Drag all of the @.m@ files in the SKTableViewDataSource group (and any sub-groups) over to "Compile Sources".

When done, just import the @.h@ files you need and you're ready to go.
...
<pre><code>cell.textLabel.text = [(Dude *)object name];<pre><code>
...
This keeps all other data in store, so 
